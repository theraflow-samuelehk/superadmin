...
import { useServiceCategories, type ServiceCategory } from "@/hooks/useServices";
...
export default function ServiceCategoryManagerDialog({ open, onOpenChange }: Props) {
  const { t } = useTranslation();
  const { categories, createCategory, updateCategory, deleteCategory } = useServiceCategories();
  ...
  const saveNew = () => {
    ...
    createCategory.mutate({ name: newName.trim(), emoji: newEmoji || null, sort_order: activeCategories.length }, {
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{t("services.manageCategories", "Gestisci categorie")}</DialogTitle>
          </DialogHeader>

          <div className="space-y-2 max-h-64 overflow-y-auto">
            {activeCategories.length === 0 && !showNew && (
              <p className="text-sm text-muted-foreground text-center py-4">{t("services.noCategories", "Nessuna categoria")}</p>
            )}
